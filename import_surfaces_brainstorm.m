function [iNewSurfaces, OutputSurfacesFiles, nVertices] = import_surfaces_Fran(iSubject, SurfaceFiles, FileFormat, isApplyMriOrient, OffsetMri, SelLabels, Comment)
% Modification introduced by Fran Lopez (CNRL, 3/8/21) for the function to
% respect the isApplyMriOrient input without asking for confirmation with the gui
% (loop created after line 138)
...
if ~isMni && isempty(isApplyMriOrient) && ~isempty(sMri) && isfield(sMri, 'InitTransf') && ~isempty(sMri.InitTransf)
    ...
    % isApplyMriOrient = 0; % Modified 3/8/21
    if isApplyMriOrient
        warning('MRI transformation IS being applied');
    else
        warning('MRI transformation NOT applied');
    end
end
