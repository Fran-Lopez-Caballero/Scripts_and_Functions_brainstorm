function varargout = process_CNRL_import_anatomy( varargin )
% PROCESS_CNRL_IMPORT_ANATOMY: Import a full anatomy folder (BrainVISA, BrainSuite, FreeSurfer, CIVET)
% Brainstorm function modified by Fran Lopez (CNRL) to import HCP parcellations within anatomy folder
% 04/06/2020

% Note to use it:
% Copy the "CNRL_import_anatomy_fs.m" and "process_CNRL_import_anatomy.m" into your USER brainstorm folder for custom functions.
% Should be something like: C:\Users\xxx\.brainstorm\process 
% The function 'CNRL import anatomy folder' should automatically appear in your GUI pipeline editor list under CNRL section. 
...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    sProcess.Comment     = 'CNRL import anatomy folder';
    sProcess.Category    = 'File';
    sProcess.SubGroup    = 'CNRL';
    sProcess.Index       = 10004;
    sProcess.Description = 'https://lmgtfy.com/?q=import+anatomy+folder+brainstorm';
...
function OutputFiles = Run(sProcess, sInputs) %#ok<DEFNU>
    ...
    switch (FileFormat)
        case 'FreeSurfer'
            errorMsg = CNRL_import_anatomy_fs(iSubject, AnatDir, nVertices, 0, sFid, 0, isAseg);
        case 'FreeSurfer+Thick'
            errorMsg = CNRL_import_anatomy_fs(iSubject, AnatDir, nVertices, 0, sFid, 1);
        case 'BrainSuite'
