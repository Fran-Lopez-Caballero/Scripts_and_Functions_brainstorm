function varargout = process_CNRL_detectbad_channel_absolute( varargin ) %#ok<*STOUT>

% Brainstorm function modified for CNRL by Fran López-Caballero (5/5/2021):

% 1) It rejects trials based on absolute amplitude threshold (rather than peak to peak)
% 2) It allows you to decide which channel do you want the rejection to be
% based on (default allows to choose between EEG, MEG, etc, this includes
% a single channel option too)
...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    sProcess.Comment     = 'Detect bad channels: absolute values (single channel)';
    ...
    % Warning
    sProcess.options.warning.Comment = ['<B>Warning</B>: Since this function  uses an absolute threshold,<BR>' ...
                                        'only include a positive value in the second entry. For instance,<BR>' ...
                                        'if willing to reject any epochs above or below 50uV, put 50 in the <BR>' ...
                                        'second text box and nothing in the first. It will reject anything<BR>' ...
                                        'above 50 or below -50 automatically.<BR><BR>'];
    ...
    % Other single channel selected
    sProcess.options.single_chan.Comment = str_pad('Single chan:');
    sProcess.options.single_chan.Type    = 'range';
    % sProcess.options.single_chan.Value   = {[0,0], 'variable', 2}; 
    sProcess.options.single_chan.Value   = {[0,0],'none', 2};% Factor 3, 6 or 15
    % Channel name comment
    sProcess.options.channelhelp.Comment = '<I><FONT color="#777777">Scale will be ft if selecting a MEG sensor, mV for ECG and &mu;V for all the others</FONT></I>';
    sProcess.options.channelhelp.Type    = 'label';
    % Explanations
    sProcess.options.comment1.Comment = ['<BR>  - <B>First column</B>: Do not add any value<BR>' 10 ...
                                         '  - <B>Second column</B>: Signal rejection threshold (absolute threshold)'];
    sProcess.options.comment1.Type    = 'label';
    % Separator
    sProcess.options.separator.Type = 'separator';
    sProcess.options.separator.Comment = ' ';
    % Channel name
    sProcess.options.channelname.Comment = 'Channel name (in case selected): ';
...
function Comment = FormatComment(sProcess)
    ...
    if (sProcess.options.rejectmode.Value == 1)
        Comment = 'Detect bad channels: absolute threshold ';
    else
        Comment = 'Detect bad trials: absolute threshold ';
    end
    % What are the criteria
    for critName = {'meggrad', 'megmag', 'eeg', 'eog', 'ecg','single_chan'}
...
function OutputFiles = Run(sProcess, sInputs) %#ok<DEFNU>
    ...
    Criteria.ecg = sProcess.options.ecg.Value{1} .* 1e-3;
    isEmpty(end+1) = all(Criteria.ecg == 0);
    % Get channel name in case of single channels
    chanName = strtrim(sProcess.options.channelname.Value);
    % Load channel file from first file
    ChannelFile_name = sInputs(1).ChannelFile;
    if isempty(ChannelFile_name)
        error('no channel file to determine units')
    else
        prov_channel_file = in_bst_channel(ChannelFile_name);
        pos_single_chan = find(strcmp({prov_channel_file.Channel.Name},chanName));
        type_chan = prov_channel_file.Channel(pos_single_chan).Type; %#ok<*FNDSB>
    end
    if contains(type_chan,'MEG') % MEG channels, ft
        Criteria.single_chan = sProcess.options.single_chan.Value{1} .* 1e-15;
        isEmpty(end+1) = all(Criteria.single_chan == 0);
    elseif strcmp(type_chan,'ECG') || strcmp(type_chan,'ecg') % ECG, mV
        Criteria.single_chan = sProcess.options.single_chan.Value{1} .* 1e-3;
        isEmpty(end+1) = all(Criteria.single_chan == 0);
    else % EEG or any others, microVolts
        Criteria.single_chan = sProcess.options.single_chan.Value{1} .* 1e-6;
        isEmpty(end+1) = all(Criteria.single_chan == 0);
    end
    % Get time window
    ...
    for iFile = 1:length(sInputs)
        ...
        Modalities = {'ECG','EEG','EEG REF','EOG','MEG GRAD','MEG MAG','Misc','Stim','single_chan'};
        % Load file
        ...
        for iMod = 1:length(Modalities)
            ...
            elseif ~isempty(strfind(lower(Modalities{iMod}), 'ecg')) || ~isempty(strfind(lower(Modalities{iMod}), 'ekg')) %#ok<*STREMP>
                Threshold = Criteria.ecg;
            elseif ismember(Modalities{iMod}, {'single_chan'})
                Threshold = Criteria.single_chan;
            else
                continue;
            end
            ...
            if strcmp(Modalities{iMod},'single_chan')
                % First, get good chans in all modalities
                iChan = good_channel(ChannelMat.Channel, DataMat.ChannelFlag, Modalities);
                % Now select within this list the ones matching the single_chan name
                pos_single_chan = find(strcmp({ChannelMat.Channel.Name},chanName));
                if any(iChan == pos_single_chan)
                    iChan = pos_single_chan;
                else
                    % Go to next modality because the single channel
                    % selected was actually a bad one
                    continue
                end
            else
...
%             p2p = (max(DataToTest,[],2) - min(DataToTest,[],2));
            % Retrieve max and min for all the sensors
            max_value = max(DataToTest,[],2);
            min_value = min(DataToTest,[],2);
            % Get bad channels
%             iBadChanMod = find((p2p < Threshold(1)) | (p2p > Threshold(2)));
            iBadChanMod = find((min_value < (Threshold(2)*-1)) | (max_value > Threshold(2)));
            % If some bad channels were detected
            if ~isempty(iBadChanMod)
                % Convert indices back into the intial Channels structure
                iBadChan = [iBadChan, iChan(iBadChanMod)]; %#ok<*AGROW>
            end
        end
...
function SetTrialStatus(FileNames, isBad)
    ...
    if ischar(FileNames)
        FileNames = {FileNames};
        [tmp__, iStudies, iDatas] = bst_get('DataFile', FileNames{1}); %#ok<*ASGLU>
